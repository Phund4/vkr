...
Точные мегабайты зависят от формата сообщений в Kafka, сжатия кадров и размера ответов. Ниже — **иллюстративная** оценка, чтобы согласовать с сетью и дисками.

| Поток | Допущение | Среднее | Пик |
|-------|------------|---------|-----|
| **Kafka**: события приёма (видео/ML) | ~10 000 сообщ/с, ~1 КБ на сообщение в среднем | ~10 МБ/с | ~14 МБ/с (`× 1,4`) |
